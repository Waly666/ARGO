...
async function listarMatriculasPrograma(idProg, query = {}, ctx = {}) {
  ...
  const items = await enrichMatriculas(mats);
  return { items, total, skip, limit };
}

function enrichMatriculas(mats) {
...
/**
 * Una matrícula por alumno (numDoc): prioriza tarifa virtual y la más reciente.
 */
async function listarMatriculasProgramaUnicas(idProg, query = {}, ctx = {}) {
  if (!idProg) {
    return { items: [], total: 0, skip: 0, limit: paginacion(query).limit };
  }
  ...
  const basePipeline = [
    { $match: filter },
    { $sort: { tarifa: -1, fechaMat: -1 } },
    { $group: { _id: '$numDoc', doc: { $first: '$$ROOT' } } },
    { $replaceRoot: { newRoot: '$doc' } },
    { $sort: { fechaMat: -1 } },
  ];

  const [countAgg, mats] = await Promise.all([
    Matricula.aggregate([...basePipeline, { $count: 'total' }]),
    Matricula.aggregate([...basePipeline, { $skip: skip }, { $limit: limit }]),
  ]);
...
module.exports = {
  listarMatriculasPrograma,
  listarMatriculasProgramaUnicas,
  TARIFA_VIRTUAL,
  etiquetaTarifa,
};
