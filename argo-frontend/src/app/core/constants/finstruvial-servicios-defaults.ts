...
function img(id: string, etiqueta: string, alt = '', promptImagen = ''): PortalFinstruvialServicioImagen {
  const altText = (alt || etiqueta).trim();
  return {
    id,
    etiqueta,
    url: '',
    alt: altText,
    promptImagen: promptImagen.trim() || promptFotoHorizontal(altText),
  };
}
...
function defaultImagenes(wire: Partial<PortalFinstruvialServicioLanding>): PortalFinstruvialServicioImagen[] {
  if (wire.imagenes?.length) {
    return wire.imagenes.map((item) => img(item.id, item.etiqueta, item.alt, item.promptImagen || ''));
  }
  return [img('hero', 'Imagen principal'), img('seccion', 'Imagen de sección')];
}

function heroAltFromWire(
  wire: Partial<PortalFinstruvialServicioLanding>,
  menuLabel: string,
): string {
  return (
    wire.imagenes?.find((item) => item.id === 'hero')?.alt?.trim() ||
    wire.heroImagenAlt?.trim() ||
    menuLabel
  );
}
...
function paginaFromWireframe(
  ...
  return {
    ...
    ribbon: [],
    heroImagenUrl: '',
    heroImagenAlt: heroAltFromWire(wire, menuLabel),
    heroImagenPrompt:
      wire.heroImagenPrompt?.trim() || promptFotoHorizontal(heroAltFromWire(wire, menuLabel)),
...
export function buildPortafolioServiciosDefaults(opts: {
  ...
  return {
    ...
    hub: {
      ...
      heroImagenPrompt:
        opts.hub?.heroImagenPrompt?.trim() ||
        promptFotoHorizontal(opts.hub?.heroImagenAlt || opts.menuLabel),
    },
...
function mergeArchivos(
  ...
  for (const imgRaw of raw || []) {
    ...
    byId.set(id, {
      ...
      promptImagen:
        str(imgRaw.promptImagen, base.promptImagen ?? '') ||
        promptFotoHorizontal(str(imgRaw.alt, base.alt)),
    });
  }
  return [...byId.values()];
}
...
export function mergeFinstruvialServicioLanding(
  ...
  return {
    ...
    heroImagenPrompt:
      str(src.heroImagenPrompt, d.heroImagenPrompt ?? '') ||
      promptFotoHorizontal(str(src.heroImagenAlt, d.heroImagenAlt)),
...
export function mergeFinstruvialServicios(
  ...
  return {
    ...
    hub: {
      ...
      formacionKicker: str(src.hub?.formacionKicker, d.hub.formacionKicker),
      gridKicker: str(src.hub?.gridKicker, d.hub.gridKicker),
      ctaPrincipal: str(src.hub?.ctaPrincipal, d.hub.ctaPrincipal),
      ctaPrincipalUrl: str(src.hub?.ctaPrincipalUrl, d.hub.ctaPrincipalUrl),
      ctaSecundario: str(src.hub?.ctaSecundario, d.hub.ctaSecundario),
      ctaSecundarioUrl: str(src.hub?.ctaSecundarioUrl, d.hub.ctaSecundarioUrl),
      statsLabel: str(src.hub?.statsLabel, d.hub.statsLabel),
      ...
      heroImagenPrompt:
        str(src.hub?.heroImagenPrompt, d.hub.heroImagenPrompt ?? '') ||
        promptFotoHorizontal(str(src.hub?.heroImagenAlt, d.hub.heroImagenAlt)),
    },
    paginas,
  };
}
