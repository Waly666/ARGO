...
const IMG = (
  ...
  alt: string,
  promptImagen: string,
): MpImagen => ({ id, etiqueta, url: '', alt, promptImagen });

export interface MpNavItem {
  id: string;
  label: string;
}

export interface MpTarjeta {
  titulo: string;
  texto: string;
  codigo?: string;
}

export interface MpClase {
  numero: number;
  titulo: string;
  texto: string;
}
...
export interface MpMito {
  mito: string;
  realidad: string;
}

export interface MpFuente {
  titulo: string;
  url: string;
  etiqueta: string;
}
...
export interface MpModoTransporte {
  icono: string;
  titulo: string;
  texto: string;
}

export interface MpHomeItem {
  numero: number;
  icon: string;
...
export interface PortalMercanciasPeligrosasLanding {
  ...
  imagenes: MpImagen[];
  queSonTitulo: string;
  queSonDefinicion: string;
  queSonRiesgos: string[];
  queSonPorQue: string;
  queSonProtege: string[];
  normativaTitulo: string;
  normativaLead: string;
  normativaDecretoTitulo: string;
  normativaDecretoTexto: string;
  normativaNtcs: MpTarjeta[];
  actualizacionTitulo: string;
  actualizacionTexto: string;
  actualizacionAviso: string;
  clasificacionTitulo: string;
  clasificacionLead: string;
  clases: MpClase[];
  identificacionTitulo: string;
  identificacionLead: string;
  identificacionBeneficios: string[];
  documentacionTitulo: string;
  documentacionLead: string;
  documentacionItems: string[];
  hojaSeguridadTitulo: string;
  hojaSeguridadTexto: string;
  hojaSeguridadAviso: string;
  tarjetaEmergenciaTitulo: string;
  tarjetaEmergenciaTexto: string;
  tarjetaEmergenciaFuncion: string;
  responsabilidadesTitulo: string;
  responsabilidadesLead: string;
  actores: MpActor[];
  capacitacionTitulo: string;
  capacitacionTexto: string;
  capacitacionMensaje: string;
  vehiculosTitulo: string;
  vehiculosLead: string;
  vehiculosAspectos: string[];
  gestionTitulo: string;
  gestionLead: string;
  gestionPasos: MpTarjeta[];
  gestionCierre: string;
  internacionalTitulo: string;
  internacionalTexto: string;
  internacionalAspectos: string[];
  modosTitulo: string;
  modosLead: string;
  modos: MpModoTransporte[];
  emergenciaTitulo: string;
  emergenciaRegla: string;
  emergenciaTexto: string;
  emergenciaPasos: string[];
  mitos: MpMito[];
  faq: MpFaq[];
  glosario: MpTarjeta[];
  ...
  imagenes: [
    IMG(
      ...
      promptFotoHorizontal(
        'Camión de carga o tanque con señalización de mercancías peligrosas en carretera de los Llanos; ambiente profesional de transporte terrestre.',
      ),
    ),
    IMG(
      'que-son',
      'Sección ¿Qué son?',
      'Capacitación en transporte de mercancías peligrosas en Colombia',
      promptFotoHorizontal(
        'Operario en bodega inspeccionando etiquetas, pictogramas y fichas de seguridad de materiales peligrosos.',
      ),
    ),
    IMG(
      'normativa',
      'Sección normativa',
      'Formación para conductores de mercancías peligrosas en SERVIAL',
      promptFotoHorizontal(
        'Mesa de trabajo con documentos legales del sector transporte y computador; ambiente de oficina técnica o cumplimiento normativo.',
      ),
    ),
    IMG(
      'clasificacion',
      'Sección clasificación',
      'Curso de mercancías peligrosas para empresas del Meta',
      promptFotoHorizontal(
        'Exhibición didáctica de clases de mercancías peligrosas con pictogramas de colores en un entorno de capacitación (sin texto legible específico).',
      ),
    ),
    IMG(
      'seguridad',
      'Sección seguridad',
      'Capacitación de mercancías peligrosas en los Llanos Orientales',
      promptFotoHorizontal(
        'Personal con equipo de emergencia ante un derrame controlado en patio logístico; sensación de prevención y respuesta segura.',
      ),
    ),
  ],
...
function mergeImagenes(raw: MpImagen[] | undefined, fb: MpImagen[]): MpImagen[] {
  ...
  return fb.map((item, i) => {
    ...
    return {
      ...
      promptImagen: (r?.promptImagen ?? item.promptImagen ?? '').trim() || item.promptImagen,
    };
  });
}
...
function mergeDocumentosGrupos(
