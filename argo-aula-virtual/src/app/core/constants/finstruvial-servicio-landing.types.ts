...
export interface PortalFinstruvialServicioBloque {
  ...
  titulo: string;
  texto: string;
  /** ID en `imagenes` (p. ej. foto1) para mostrar junto al texto. */
  imagenId?: string;
  /** URL de YouTube para incrustar junto al texto del bloque. */
  youtubeUrl?: string;
...
export interface PortalFinstruvialServicioLanding {
  ...
  /** URL de YouTube (watch, youtu.be o embed). Se muestra debajo del hero. */
  ...
  /** Videos tutoriales (registro, matrícula, pago) dentro de «Cómo funciona». */
  ...
  /** Video de YouTube junto al texto de la sección producto / experiencias. */
  productoVideoYoutubeUrl: string;
  /** ID en `imagenes` para foto junto a la sección producto. */
