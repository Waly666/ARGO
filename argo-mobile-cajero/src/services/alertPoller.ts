...
async function pollOnce(): Promise<void> {
  ...
  if (tienePermiso(ctx.user.permisos, 'caja.turno') && habilitada('alarmas.caja.cerrada')) {
    try {
  ...
  if (tienePermiso(ctx.user.permisos, 'caja.admin') && habilitada('alarmas.caja.descuadres')) {
    try {
