...
import { useVoiceField } from '../voice/VoiceContext';
import {
  DIAS_CORTO,
  MESES_CORTO,
  MESES_LARGO,
...
export function FechaPickerField({
  ...
  useVoiceField(voiceFieldId, {
    focus: abrir,
    getValue: () => value,
    setValue: (t) => {
      const p = parseYmd(t);
      if (p) {
        const ymd = ymdFromParts(p.y, p.m, p.d);
        if (isYmdInRange(ymd, minYmd, maxYmd)) onChange(ymd);
        return;
      }
      const d8 = String(t || '').replace(/\D/g, '');
      if (d8.length === 8) {
        const ymd = `${d8.slice(0, 4)}-${d8.slice(4, 6)}-${d8.slice(6, 8)}`;
        if (parseYmd(ymd) && isYmdInRange(ymd, minYmd, maxYmd)) onChange(ymd);
      }
    },
  });
