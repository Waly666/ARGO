...
const K_REMEMBER = 'argo_jor_remember_login';
const K_USER = 'argo_jor_saved_user';
const K_PASS = 'argo_jor_saved_pass';
...
export async function loadSavedLogin(): Promise<SavedLogin> {
  ...
  if (!remember) return { remember: false, username: '', password: '' };
  return {
    remember: true,
